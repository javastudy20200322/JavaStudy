package practice.week10.jaeyeon;
...
public class ThreadPoolExample {
	public static void main(String[] args) {
		...
		for (int j = 0; j < 10; j++) {
	//.start();;
			executorService.execute(new Thread(new Calculate(customerList.get(j))));
		}
...
class Calculate implements Runnable {
	...
	Calculate(int cashier, String customer) {
	...
	public synchronized void run() {
		try {
			...
			System.out.println(
					"\t\t\t\t\t\t\t\t" + Thread.currentThread().getName() + " : " + this.customer + "의 계산을 종료.");
		}
	}
}
